...
def parse_news_items(generate_path):
    ...
    # 解析元组（兼容 4 元组或 5 元组 + v19 术语注解）
    #   4 元组: (idx, news, source, signal)
    #   5 元组: (idx, news, source, signal, term_note)
    items = []
...
def parse_v32_page3_data(page3_path):
    ...
    # DEPTH list（每项含 label / title / detail）·支持类型注解（DEPTH: list[dict] = [）
    ...
    # ACTIONS list（4 项 tuple: label / target / note / tag）·支持类型注解（ACTIONS: list[tuple[...]] = [）
    act_match = re.search(r'ACTIONS\s*[:=][^=]*?\[(.*?)\n\]', content, re.DOTALL)
    if act_match:
...
def check_03_24h_strict(items, today=None):
    """坑 3：NEWS 事件日期 today-1 → 超 24h（红线 11 24h 严格执行·2026-09-13 升级）

    v10 旧版：delta > 2 fail / delta > 1 边界（49h 内合规）→ 与业务红线 11（24h 严格）不一致。
    2026-09-12 实战发现 v7 用 9/11 6 条报"国内边界"失败，但业务红线 11 允许 9/11（24h 内）。
    本版升级：delta ≤ 1 = 24h 内 pass / delta == 2 = 边界（still pass，留红字提示）
    / delta > 2 = 硬 fail。
    """
    c = Check("03", "事件日期超 24h", "v10")
    if today is None:
        today = date.today()
    ...
    for item in items:
        ...
        try:
            ...
            if delta > 2:
                ...
                boundary.append(f"⚠️ 条 {item['idx']}: {item['source']} ({delta} 天前 · 边界)")
        except (ValueError, TypeError):
            pass

    if expired:
        c.fail(f"❌ {len(expired)} 条超 24h → 必替换\n   " + "\n   ".join(expired))
    else:
        detail = "✓ 所有日期均在 24h 内"
        if boundary:
            detail += f"\n   边界提示：{len(boundary)} 条 2 天前（合规·但建议换更近的）"
            detail += "\n   " + "\n   ".join(boundary)
        c.pass_(detail)
    return c
...
def check_29_v38_report_knowledge_increment(project_dir):
    ...
    # 兼容两种命名:数据真伪核对报告.md / 数据真实性核对报告.md
    report_files = [f for f in os.listdir(project_dir) if ('核对' in f or '真伪' in f or '真实性' in f) and f.endswith('.md')]
    if not report_files:
        c.fail("❌ 数据真伪核对报告.md 不存在")
        return c
    report_path = os.path.join(project_dir, report_files[0])
...
def check_30_v38_fix_three_steps(project_dir):
    ...
    # 兼容两种命名
    report_files_30 = [f for f in os.listdir(project_dir) if ('核对' in f or '真伪' in f or '真实性' in f) and f.endswith('.md')]
    if report_files_30:
        report_path = os.path.join(project_dir, report_files_30[0])
